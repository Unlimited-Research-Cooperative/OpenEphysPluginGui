...
void SourceNode::process(AudioBuffer<float>& buffer)
{
    ...
    std::string elapsedTimeStr = std::to_string(elapsedTime) + " ms";
    
    // Create a ZeroMQ message from the string
    zmq::message_t message(elapsedTimeStr.begin(), elapsedTimeStr.end());
    
    // Send elapsed time via ZeroMQ
    socket.send(message, zmq::send_flags::none);
    
    // Print elapsed time to the console
    DBG("Time taken for processing: " << elapsedTime << " ms");
}